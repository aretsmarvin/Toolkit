...
class ExternalToolConfig:
    ...
    command: List[str] | None = None
    install: dict | None = None        # passed straight to InstallWorker
...
def load_config() -> AppConfig:
    ...
    log_dir_raw = raw.get("logging", {}).get("log_dir", "~/.toolkit/logs")
    ...
    for item in raw.get("external_tools", []):
        ...
        update = None
        if isinstance(update_raw, dict) and update_raw:
